...
def init_to_zero(name):
    def hook(nargs):
        if name in nargs:
            ...
            print(f"Available keys in nargs: {list(nargs.keys())}")
            print(f"Warning: '{name}' not found in nargs")
    return hook
...
def get_configs_io_bound() -> List[Config]:
    ...
    #             for block_n in [32, 64, 128, 256]:
                    ...
                    # split_k
                    for split_k in [2, 4, 8, 16]:  # Disabled [2, 4, 8, 16]:
                        configs.append(
                            Config(
                                {
                                    "BLOCK_M": block_m,
...
MATMUL_CONFIGS: List[Config] = get_configs_io_bound()
# [
...
def _kernel_matmul_fp8_row(
    ...
    for _ in range(0, k_tiles * tiles_per_SM):
        ...
        _A = A + (offs_am[:, None] * stride_am + offs_k[None, :] * stride_ak)
        _B = B + (offs_k[:, None] * stride_bk + offs_bn[None, :] * stride_bn)

        a = tl.load(_A, mask=offs_k_for_mask[None, :] < K - ki * BLOCK_K, other=0.0)
        b = tl.load(_B, mask=offs_k_for_mask[:, None] < K - ki * BLOCK_K, other=0.0)
        ...
        if ki == k_tiles - 1:
            ...
            _C = C + (rm[:, None] * stride_cm + rn[None, :] * stride_cn)
            mask = (rm < M)[:, None] & (rn < N)[None, :]
            # Handles write-back with reduction-splitting
            tl.store(_C, acc, mask=mask)
            acc = tl.zeros((BLOCK_M, BLOCK_N), dtype=dot_out_dtype)
...
def matmul_fp8_row(
    ...
    if no_use_persistent:
        ...
        # print("elif fp8_fast_accum: ")
        # print("a.dtype: ", a.dtype)
        # print("b.dtype: ", b.dtype)
        # print("c.dtype: ", c.dtype)
        # print("a_scale.dtype: ", a_scale.dtype)
        # print("b_scale.dtype: ", b_scale.dtype)
