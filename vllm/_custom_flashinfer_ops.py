...
if current_platform.is_rocm():
    try:
        import vllm._rocm_flashinfer_C
    except ImportError as e:
        logger.warning("Failed to import from vllm._rocm_flashinfer_C with %r", e)
...
def _sampling_from_probs(
    ...
    with probs.device as device:
        ...
        torch.ops._rocm_flashinfer_C.sampling_from_probs(
            probs,
            uniform_samples,
            samples,
            deterministic,
...
def _top_p_sampling_from_probs(
    probs: torch.Tensor,
    uniform_samples: torch.Tensor,
    ...
    with probs.device as device:
        ...
        torch.ops._rocm_flashinfer_C.top_p_sampling_from_probs(
            probs,
            uniform_samples,
            samples,
            success,
...
def _top_k_sampling_from_probs(
    probs: torch.Tensor,
    uniform_samples: torch.Tensor,
    ...
    with probs.device as device:
        ...
        samples = torch.empty(probs.size(0), dtype=torch.int32, device=device)
        success = torch.empty(probs.size(0), dtype=torch.bool, device=device)
        torch.ops._rocm_flashinfer_C.top_k_sampling_from_probs(
            probs,
            uniform_samples,
            samples,
            success,
...
def _min_p_sampling_from_probs(
    probs: torch.Tensor,
    uniform_samples: torch.Tensor,
    ...
    with probs.device as device:
        ...
        samples = torch.empty(probs.size(0), dtype=torch.int32, device=device)
        success = torch.empty(probs.size(0), dtype=torch.bool, device=device)
        torch.ops._rocm_flashinfer_C.min_p_sampling_from_probs(
            probs,
            uniform_samples,
            samples,
            success,
...
def _top_k_top_p_sampling_from_probs(
    probs: torch.Tensor,
    uniform_samples: torch.Tensor,
    maybe_top_k_arr: Optional[torch.Tensor],
    top_k_val: int,
    ...
    with probs.device as device:
        ...
        samples = torch.empty(probs.size(0), dtype=torch.int32, device=device)
        success = torch.empty(probs.size(0), dtype=torch.bool, device=device)
        torch.ops._rocm_flashinfer_C.top_k_top_p_sampling_from_probs(
            probs,
            uniform_samples,
            samples,
            success,
...
def _top_p_renorm_probs(
    ...
    with probs.device as device:
        ...
        torch.ops._rocm_flashinfer_C.top_p_renorm_probs(
            probs,
            renorm_probs,
            maybe_top_p_arr,
            top_p_val,
...
def _top_k_renorm_probs(
    ...
    with probs.device as device:
        ...
        renorm_probs = torch.empty_like(probs)
        torch.ops._rocm_flashinfer_C.top_k_renorm_probs(
            probs,
            renorm_probs,
            maybe_top_k_arr,
            top_k_val,
...
def _top_k_mask_logits(
    ...
    with logits.device as device:
        ...
        torch.ops._rocm_flashinfer_C.top_k_mask_logits(
            logits,
            mask_logits,
            maybe_top_k_arr,
            top_k_val,
...
def _chain_speculative_sampling(
    ...
    with draft_probs.device as device:
        ...
        torch.ops._rocm_flashinfer_C.chain_speculative_sampling(
            draft_probs,
            draft_token_ids,
            uniform_samples,
            target_probs,
...
if hasattr(torch.ops._rocm_flashinfer_C, "sampling_from_probs"):
    
    @register_fake("_rocm_flashinfer_C::sampling_from_probs")
    def _sampling_from_probs_fake(
        probs: torch.Tensor,
        uniform_samples: torch.Tensor,
        deterministic: bool,
    ) -> torch.Tensor:
    ...
    @register_fake("_rocm_flashinfer_C::top_p_sampling_from_probs")
    def _top_p_sampling_from_probs_fake(
        probs: torch.Tensor,
        uniform_samples: torch.Tensor,
        maybe_top_p_arr: Optional[torch.Tensor],
        top_p_val: float,
    ...
    @register_fake("_rocm_flashinfer_C::top_k_sampling_from_probs")
    def _top_k_sampling_from_probs_fake(
        probs: torch.Tensor,
        uniform_samples: torch.Tensor,
        maybe_top_k_arr: Optional[torch.Tensor],
        top_k_val: int,
    ...
    @register_fake("_rocm_flashinfer_C::min_p_sampling_from_probs")
    def _min_p_sampling_from_probs_fake(
        probs: torch.Tensor,
        uniform_samples: torch.Tensor,
        maybe_min_p_arr: Optional[torch.Tensor],
        min_p_val: float,
    ...
    @register_fake("_rocm_flashinfer_C::top_k_top_p_sampling_from_probs")
    def _top_k_top_p_sampling_from_probs_fake(
        probs: torch.Tensor,
        uniform_samples: torch.Tensor,
        maybe_top_k_arr: Optional[torch.Tensor],
        top_k_val: int,
    ...
    @register_fake("_rocm_flashinfer_C::top_p_renorm_probs")
    def _top_p_renorm_probs_fake(
        probs: torch.Tensor,
        maybe_top_p_arr: Optional[torch.Tensor],
        top_p_val: float,
    ) -> torch.Tensor:
    ...
    @register_fake("_rocm_flashinfer_C::top_k_renorm_probs")
    def _top_k_renorm_probs_fake(
        probs: torch.Tensor,
        maybe_top_k_arr: Optional[torch.Tensor],
        top_k_val: int,
    ) -> torch.Tensor:
    ...
    @register_fake("_rocm_flashinfer_C::top_k_mask_logits")
    def _top_k_mask_logits_fake(
        logits: torch.Tensor,
        maybe_top_k_arr: Optional[torch.Tensor],
        top_k_val: int,
    ) -> torch.Tensor:
    ...
    @register_fake("_rocm_flashinfer_C::chain_speculative_sampling")
    def _chain_speculative_sampling_fake(
        draft_probs: torch.Tensor,
        draft_token_ids: torch.Tensor,
        uniform_samples: torch.Tensor,
        target_probs: torch.Tensor,
...
def top_p_sampling_from_probs(
    ...
    if check_nan:
        if torch.any(torch.isnan(probs)):
            raise ValueError("Input probs contains NaN.")
    return _top_p_sampling_from_probs(
...
def top_k_sampling_from_probs(
    ...
    if check_nan:
        if torch.any(torch.isnan(probs)):
            raise ValueError("Input probs contains NaN.")
    return _top_k_sampling_from_probs(
...
def min_p_sampling_from_probs(
    ...
    if check_nan:
        if torch.any(torch.isnan(probs)):
            raise ValueError("Input probs contains NaN.")
    return _min_p_sampling_from_probs(
...
def top_k_top_p_sampling_from_logits(
    ...
    if filter_apply_order == "top_k_first":
        ...
        if check_nan:
            if torch.any(torch.isnan(probs)):
                raise ValueError("Input probs contains NaN.")
        return _top_k_top_p_sampling_from_probs(
            probs,
            uniform_samples,
...
def chain_speculative_sampling(
    ...
    output_token_ids = _chain_speculative_sampling(
        draft_probs,
        draft_token_ids,
        uniform_samples,
        target_probs,
