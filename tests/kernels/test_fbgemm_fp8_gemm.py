...
if not current_platform.is_rocm():
    pytest.skip(
        reason=
        "FBGEMM Kernel currently only supported on ROCm through CK kernel.",
...
def get_cond_label(M, N, K):
    ...
    #     return "COND_4"
        ...
        return "COND_5"
    elif ((M < 512 and K < 8192) or (N <= 2048 and K <= 8192) or
          (K <= 2048 and N <= 8192)) and K >= 1024:  # COND_6
        return "COND_6"
    elif K < 1024:  # COND_7
        return "COND_7"
    elif M < 1024:  # COND_8
...
def test_f8f8bf16_rowwise_opcheck(M, N, K):
    ...
    # X = (M, K)
    # W = (N, K)
    opcheck(torch.ops._fp8gemm_C.f8f8bf16_rowwise,
...
def test_f8f8bf16_rowwise(M, N, K):
    ...
    # X = (M, K)
    # W = (K, N)
    ref_output = vec_scaled_mm_torch(XQ,
                                     WQ.transpose(1, 0),
                                     x_scale,
                                     w_scale,
    ...
    # X = (M, K)
    # W = (N, K)
    output = torch.ops._fp8gemm_C.f8f8bf16_rowwise(XQ, WQ, x_scale, w_scale,
                                                   None, True)
