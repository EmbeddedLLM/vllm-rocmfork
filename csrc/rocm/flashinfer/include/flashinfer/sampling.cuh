...
#define _LAUNCH_KERNEL cudaLaunchKernel((void*)kernel, nblks, nthrs, args, smem_size, stream)
#else
using namespace hipcub;
namespace cub = hipcub;
using std::numeric_limits;
#define _LAUNCH_KERNEL hipLaunchKernel((void*)kernel, nblks, nthrs, args, smem_size, stream)
#endif
...
#else
    max_p = max(max_p, BlockReduce<DType, BLOCK_THREADS>(temp_storage.block_prim.reduce)
                           .template Reduce<VEC_SIZE>(probs_, hipcub::Max()));
#endif
    __syncthreads();
  }
  if (tx == 0) {
    ...
    DISPATCH_ALIGNED_VEC_SIZE(
        vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
          auto kernel = TopKSamplingFromProbKernel<BLOCK_THREADS, SCAN_ALGO, REDUCE_ALGO, VEC_SIZE,
                                                   DETERMINISTIC, T, IdType>;
          FLASHINFER_CUDA_CALL(
              VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize((void*)kernel, smem_size));
          FLASHINFER_CUDA_CALL(
              _LAUNCH_KERNEL);
        })});
    return cudaSuccess;
...
#pragma unroll
      ...
      min_gt_low =
          BlockReduce<DType, BLOCK_THREADS, REDUCE_ALGORITHM>(temp_storage.block_prim.reduce)
              .template Reduce(min_gt_low, cub::Min());
      __syncthreads();
      max_le_high =
          BlockReduce<DType, BLOCK_THREADS, REDUCE_ALGORITHM>(temp_storage.block_prim.reduce)
              .template Reduce(max_le_high, cub::Max());
      if (tx == 0) {
...
cudaError_t TopPRenormProb(DType* probs, DType* renormed_prob, DType* top_p_arr,
  ...
  DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {
    ...
    FLASHINFER_CUDA_CALL(
        VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize((void*)kernel, smem_size));
    FLASHINFER_CUDA_CALL(_LAUNCH_KERNEL);
  });
  return cudaSuccess;
}
