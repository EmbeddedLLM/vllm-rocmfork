...
FLASHINFER_INLINE __nv_bfloat162 __float2bfloat162_rn(const float x) {
  return __hip_bfloat162{__float2bfloat16(x), __float2bfloat16(x)};
}
#endif
...
__device__ void fast_dequant_f8f16x4(uint32_t* input, uint2* output) {
  ...
  if constexpr (std::is_same_v<fp8_dtype, __nv_fp8_e5m2> && std::is_same_v<fp16_dtype, half>) {
    ...
    constexpr int FP8_MASK1 = 0x80000000;
    constexpr int FP8_MASK2 = FP8_MASK1 >> (FP8_EXPONENT + FP8_MANTISSA);
    constexpr int FP8_MASK3 = FP8_MASK2 & 0x7fffffff;
    constexpr int FP8_MASK = FP8_MASK3 | (FP8_MASK3 >> 16);
    q = __byte_perm(q, q, 0x1302);

    // Extract and shift FP8 values to FP16 format
    uint32_t Out1 = (q & 0x80008000) | ((q & FP8_MASK) >> RIGHT_SHIFT);
    uint32_t Out2 = ((q << 8) & 0x80008000) | (((q << 8) & FP8_MASK) >> RIGHT_SHIFT);
...
#else
      dst[i] = *reinterpret_cast<__nv_fp8_e5m2*>(__hip_cvt_halfraw_to_fp8(static_cast<__half_raw>(src[i]), __HIP_SATFINITE, __HIP_E5M2));
#endif
    }
#endif  // FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
  }
